...
from search.musicbrainz.utils.check_match import check_title_match, check_album_artist_title_match
    ...
    try:
        for recording in result['recording-list']:
            ...
            if len(title.split(' ')) >= 3:
                ...
                if check_album_artist_title_match(recording, fragments):
                    if recording_score >= score_cache:
                        score_cache = recording_score
                        recording_to_return = recording
                    ...
                    logging.warning(f"Couldn't validate title {title} using second method. Skipping to next iteration")
                    continue
